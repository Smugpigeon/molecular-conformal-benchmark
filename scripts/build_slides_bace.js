...
// ===================================================== 5 MAIN BENCHMARK
{
  ...
  s.addNotes(`【~70s】第一条证据，点预测。TabPFN 误差最低、相关最高。但我没停在点估计。这里要区分两类检验：基于 n=303 个测试分子的临界差异图把逐分子误差当配对块，分子并非独立重复，所以我只把它当描述性可视化，不当严格显著性。严格的显著性来自两条：逐分子配对 bootstrap（10k 重采样 + Bonferroni）给 ΔMAE 置信区间，能进一步分开 RF、GBM 与 TabPFN；以及论文级的折级 Nadeau–Bengio 校正检验，以折为重复单元、避免伪重复。综合的严谨说法是 TabPFN 与图网络、化学大模型同属第一梯队。`);
}
...
{
  const s = figslide("外推泛化", "TabPFN 在本 BACE scaffold split 中外推最稳，全参微调的 MolFormer 明显退化", "fig_d_scaffold.png",
    "预定义 vs 骨架严格划分的绝对 MAE；TabPFN 两 split 都最低、差距最小",
    `【~60s】第二条证据，也是最重要的——外推泛化。预定义划分有约 64% 骨架重叠，会高估泛化。我按 Murcko 骨架严格重划、训练和测试无共享骨架，全部重训。鲁棒性差异很大：TabPFN 几乎不退化，误差只升 0.01、相关甚至上升，在本 BACE scaffold split 中表现最稳；而全参微调的 MolFormer 泛化性能明显退化、误差升 0.46。这和点预测、误差解剖指向同一条主线：TabPFN 在点预测上最佳、在 scaffold 外推与低 AD 覆盖上最稳，整体属第一梯队。这对真实虚拟筛选最关键，因为筛的就是没见过的新分子。`, 8.3);
...
// ===================================================== 9 UNCERTAINTY
{
  const s = figslide("不确定性", "TabPFN 的不确定性可预警高误差风险、支持拒识", "fig_d_uncertainty.png",
    "按不确定性拒识 → 保留集 MAE 下降（vs 随机拒识）",
    `【~50s】既然误差集中在难分子上，能不能用不确定性预警这类高误差风险？能。TabPFN 给的预测区间宽度（预测时即可得）和实际误差正相关；更直接的——58% 的共识难例落在它的 90% 区间之外，而容易预测的分子只有 6%。所以在虚拟筛选里，把最不确定的那批先拒掉交人工或实验复核，保留集误差会明显下降——红线明显低于随机拒识。需要说明：这里"难例"是按测试误差事后定义的，但区间宽度本身在预测时就可用于拒识。这把"模型什么时候别信它"变成可操作的。`, 8.3);
...
// ===================================================== 11 CONFORMAL — AD CONDITIONAL
{
  const s = figslide("可信度 (2/2)", "但低相似度（新化学）分子明显欠覆盖", "fig_d_adcov.png",
    ...
    `【~55s】把覆盖率按到训练集的相似度分层，隐患就暴露了：8 个模型里有 6 个在低相似度、也就是新骨架子群上系统性欠覆盖、有的低到 0.2 到 0.5，而高相似度子群过覆盖——两者在总体上相互抵消，让边际覆盖"看起来"达标。也就是边际有效不等于处处有效。在低相似度上仍接近 0.9 的有两个：TabPFN 和 MolFormer——但 MolFormer 是靠区间显著更宽换来的，TabPFN 则同时保持紧致，在本数据与该分层中、于覆盖与宽度的折中上表现最好。这对外推到新化学的虚拟筛选是直接的可信度警示。需要说明的是，低相似度子群样本量较小（n=10、属探索性观察），但欠覆盖的方向在八个模型上完全一致。`, 8.3);
...
// ===================================================== 13 PBCNet2 (negative)
{
  const s = figslide("结构-成对模型", "结构-成对模型在对接位姿上未超过配体模型（可能受位姿质量影响）", "fig_d_pbcnet.png",
    ...
    `【~65s】配体模型在悬崖上集体失手，而 PBCNet2 是郑明月老师课题组的结构-成对模型、专门预测一对配体的相对活性、为悬崖场景而生——我参与了该工作的部分方法开发和代码实现，因此对 PBCNet2 的适用边界也比较关注。我在 161 个活性悬崖对上检验它，用相对活性排序的 |Spearman ρ|（已按 ΔΔG 符号对齐）衡量。结果是一个阴性结果：它在悬崖对上只有 0.375、明显低于配体模型的 0.78；我还做了约束对接的公平复测、把位姿对齐到一致参考系，仍未改变结论。可能的主因是 cross-docking 位姿质量、而非 PBCNet2 本身——它在自己的 FEP 基准上很强，但那需要晶体级共晶位姿，而 MoleculeNet 这种多样配体只能批量对接、位姿不够可靠。这个否定结果界定了结构-成对模型该在什么条件下用。`, 8.3);
...
{
  const s = content("稳健性与可解释性", "换指纹不改模型排名；模型学到可学习的结构-活性相关信号");
...
// ===================================================== 15 EVALUATION AXES
{
  ...
  s.addNotes(`【~50s】BACE 作业是回归，但我把回归预测派生出两条额外评估轴，顺带演练第五讲的全套指标。分类轴：在 100 纳摩阈值二分，Chemprop 图网络 ROC-AUC 0.915、MCC 0.650 最高，TabPFN 次之——一个有意思的细节是，回归点估计最好的 TabPFN 在分类上略输给 Chemprop，说明评估口径会改变谁第一，所以我多轴并看。早期富集轴模拟先导优化里“能否把最强的分子排到最前”：以最高活性 10% 为命中，各模型在表头富集 4 到 7 倍于随机，ChemFM 和随机森林在 EF@10% 上领先。富集曲线放在 backup。`);
}
...
// ===================================================== 17 TAKE-HOME + LIMITATIONS
{
  ...
  card(s, 0.6, 1.65, 6.0, 4.8, "Take-home", [
    ...
    b("误差系统性集中在悬崖 / 标签冲突 / 新骨架；不确定性可预警高误差风险、支持拒识。"),
  ...
  s.addNotes(`【收尾 ~45s】三个 take-home：小样本分子回归上，零调参的 TabPFN 在点预测、外推、可信度三轴均达第一梯队、并在外推与可信度上最稳；误差系统性集中在悬崖和新骨架、不确定性能预警高误差风险；结构方法未超过配体模型（可能受位姿质量影响）、我诚实报告了否定结果。限制也讲清楚：单数据集、预定义划分有泄漏（已用 scaffold split 缓解）、PBCNet2 检验可能受位姿质量影响、且没做前瞻性实验。最大的收获是把一个问题做到能 defend、并诚实面对否定结果。谢谢老师，请批评指正。`);
}
