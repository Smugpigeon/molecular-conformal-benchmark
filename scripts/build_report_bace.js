...
push(B("课程内容对应：", "本项目遵循课程项目要求，完成“数据整理→模型构建→训练→分析”的完整链路；实现上用 RDKit + scikit-learn / Chemprop / HuggingFace Transformers 在同一数据任务上复现。各课程环节与本项目的对应关系如下表。"));
...
push(H2("3. 逐分子误差解剖"));
push(P("误差并非随机：所有模型都“回归到均值”（低估高活性、高估低活性，TabPFN 收缩最小）；误差最大的十分位分子中，19% 落在活性悬崖（其余仅 1%）、16% 为标签冲突骨架（其余 2%），且更偏新骨架与极端活性（图 8）；跨 8 模型共识最难的分子多为大环、碎片样小分子或极端活性（图 9）。调参只缩小误差幅度、不改变“谁难”（大误差集合重叠 76%）。TabPFN 的预测区间宽度可预警高误差风险、支持拒识策略（难例按测试误差事后定义，但区间宽度在预测时即可用）：58% 的共识难例落在其 90% 区间之外（易预测分子仅 6%，图 10）。"));
...
push(P("分类轴（表 3、图 18）：以 pIC50≥7（IC50≤100 nM；测试集 43.9% 为活性、类别均衡）二分，用连续预测 pIC50 作打分计算 ROC-AUC / PR-AUC，在阈值 7 处二值化算 F1 / MCC。Chemprop 图网络给出最高 ROC-AUC（0.915）与 MCC（0.650），TabPFN 次之（0.900 / 0.631）。一个值得注意的细节：回归点估计最优的 TabPFN 在分类轴上略逊于 Chemprop，没有单一模型在所有评估轴上都最优，评估口径会改变“谁最好”的结论。"));
...
push(bullet("零调参的表格基础模型 TabPFN 在点预测上为最佳点估计，在 scaffold 外推与低 AD 条件覆盖上表现最稳，整体属于第一梯队；它与 Chemprop、RF、ChemFM 同属第一梯队。显著性依检验口径而定：在逐分子配对 bootstrap 与主分析 ρ=1/9 的 Nadeau–Bengio 检验下，TabPFN 显著优于线性/核模型与 RF/GBM；但在保守 ρ=0.25 NB-Holm 校正下，仅全参微调的 MolFormer 仍显著更差，其余差异不显著。"));
push(bullet("误差不是随机的：高度集中在活性悬崖、标签冲突、新骨架与极端活性分子；调参只缩小幅度、不改变“谁难”；不确定性可预警高误差风险、支持拒识策略，使“何时降低预测可信度”变得可操作。"));
...
push(bullet("可解释性显示模型抓到的是可学习的结构-活性相关信号：分子大小 / 复杂度与若干增效 / 致弱子结构主导预测，而非记忆噪声（与附录阴性对照相互印证）。"));
push(B("方法论思考：", "本实验最看重的不是优化单一数值指标，而是让每个结论都能被质疑也能被辩护：用阴性对照排除随机标签记忆、用 scaffold split 量化并披露相似性泄漏、用统计检验约束“显著”一词、对结构方法的阴性结果不作过度归因。这是把“看起来很准”升级为“知道它何时可信”的关键。"));
...
// 结论
push(H1("结论"));
push(P("在小样本 BACE-1 抑制活性预测上，零调参的表格基础模型 TabPFN 给出最优点估计，并在统计显著性、骨架外推鲁棒性、conformal 条件覆盖三条轴上对新化学的泛化均达第一梯队或最稳；模型误差系统性集中在活性悬崖、标签冲突与新骨架，且可被不确定性预警（支持拒识策略）；结构方法（对接、PBCNet2）在 cross-docking 位姿域未能超过配体模型，构成一个界定适用范围的阴性结果。全程以阴性对照、泄漏审计与显著性检验自检，结论既可被质疑也可被辩护。"));
